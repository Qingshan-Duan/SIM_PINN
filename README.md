...
| `1D_1phase/pinn_param/` | 参数化 PINN：一个网络 `(x̂,t̂,q̂)→p̂` 覆盖 ±20% 流量范围，纯物理无数据。对解析真解跨 q R²≈0.9999，追平单流量基线 | ⏳ 推进中 |
| `1D_1phase/surrogate/` | 1D 单相代理模型：扰动井控生成数据 + 物理损失正则（时变井控走自回归） | 🕐 计划中 |
